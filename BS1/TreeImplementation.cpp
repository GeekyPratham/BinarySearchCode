...
int sumOfTreeNode(Node* a){
    
    if(a==NULL) return 0;
    // int sum = a->val + sumOfTreeNode(a->left) +sumOfTreeNode(a->right);
    // return ans;
    return  a->val + sumOfTreeNode(a->left) +sumOfTreeNode(a->right);
}
...
int maximum(Node* a){
    ...
    int mx = max(a->val,max(maximum(a->left),maximum(a->right)));
    return mx;
}
int minimum(Node* a){
